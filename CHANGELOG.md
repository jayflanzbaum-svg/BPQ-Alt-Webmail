...
### Added
- **The address book is now a three-pane workspace, not a dialog.** It was a single 640px column; it is now `min(1600px, 96vw)` by `min(1000px, 92vh)` — groups rail | contact list | contact detail, the layout Thunderbird and Apple Contacts use, so a large book and a long group list both stay legible. The groups rail has its own filter box; the contact list has a Select-all header, an inline count and a 400-row render cap (with a "narrow the filter" note) so a five-figure book cannot lock the UI up rebuilding its innerHTML on every keystroke. On a phone the three panes take turns behind a Groups / Contacts / Edit switcher — tapping a group jumps to its contacts, tapping a contact jumps to the editor.
- **A full contact card, opened as a dialog over the address book.** Callsign, full name, street, city, state, ZIP, phone, email, website, free-text notes, and a group tick-box per group. It replaces the fixed editor pane: adding a contact is an occasional act, and a permanent form was taking up a third of the window for it — the contact list gets that space back. Reached from **+ Add Contact** in the list toolbar, or by clicking any contact. Older records simply have the new fields blank.
- **A chip + type-ahead group picker**, on the contact card and in the importer. Groups the contact belongs to show as removable chips; one input filters the list as you type, and a name that does not exist yet is offered as **+ Create “…”** — so making a group is a side effect of typing it, with no trip back to the address book and no extra dialog. Clicking the empty field drops down every group, so with four groups it still reads like a list, while forty or four hundred are reached by typing instead of scrolling. Full keyboard support: ↑↓ to move, Enter to take the highlight (or create what you typed), Backspace on an empty field to drop the last chip, Escape to close. The menu is positioned like the context menu so the card's own scrolling body cannot clip it. On an existing contact a change applies immediately; on a new one it applies on Save, pre-loaded with whatever group is open. This replaced a fixed checkbox well, which would have become a cramped 150px scroll area once someone had more than a handful of groups. Selecting a contact opens it on the right with a tick-box per group, which is the direct way to answer "which groups is this operator in?" — the bulk bar answers the opposite question. For an existing contact the ticks write through immediately; for a new one they apply on Save, pre-ticked with whatever group is currently open.
- **Import contacts — paste a list or load a CSV, and file it into groups in one pass.** Target groups are chosen with the same picker as the contact card, so a group can be created straight from the import. Accepts callsigns separated by anything (spaces, commas, semicolons, newlines) or a CSV, with or without a `callsign,name,city,state` header row, including quoted `"Surname, First"` fields. A list of nothing but callsigns is read as a list even when comma-separated; a row whose later fields are *not* callsigns is read as CSV columns. Routing suffixes are stripped, duplicates collapse, and anything that is not callsign-shaped is skipped and named in the preview rather than silently dropped. Before committing, the preview states exactly how many are new versus already known and which groups they will land in. Existing contacts are never clobbered — an import only fills in blank fields. Target groups are picked as chips, plus an optional new group created on the spot. A CSV can also be dropped straight onto the textarea.
- **QRZ now fills the street, ZIP and email too**, not just name/city/state — the card has somewhere to put them. A lookup only ever fills blank fields, so nothing you typed is overwritten, and the status line names exactly which fields it filled.
- **Optional QRZ enrichment after an import**, walking the new contacts one per second (it is a paid, rate-limited API and an import can be hundreds of calls), with progress in the footer and a summary at the end.
- **✉ Compose in the address-book toolbar**, beside Import and + Add Contact. It addresses the contacts you have ticked (the button carries the count, e.g. “✉ Compose (3)”) or, when nothing is ticked and a group is open, everyone in that group. Recipients are de-duplicated and sorted, so the To field is the same whatever order they were picked in.
- **Bulk delete** in the contact list's selection bar, alongside add-to-group and remove-from-group.
- **Contact groups in the address book.** A group is a named list of callsigns (`bpq_groups` in localStorage) that *references* contacts rather than copying them, so editing a contact once updates it everywhere and a contact can belong to any number of groups — the Gmail-label model rather than Outlook's copy-into-the-list one, which scales better in a small pane. The modal is now two-pane: a groups rail on the left (All Contacts + every group, with member counts) and the contact list on the right, mirroring Apple Contacts and new Outlook's "your contact lists" side panel. Membership is edited by ticking contacts and using **Add to group** / **Remove from group** (Gmail's select-then-label gesture), or by dragging a contact onto a group in the rail (Thunderbird's drag-onto-the-list gesture). Each contact row shows chips for the groups it is in. Right-clicking a group offers Compose / Rename / Delete; deleting a group never deletes contacts, and deleting or renaming a contact is propagated into every group.
- **Send to a group.** Right-click a group → *Compose to …* fills the To field with every member. BPQ's `EMSave` takes one recipient per POST, so `doSend()` now splits a comma/semicolon To list and sends once per recipient, with a confirmation first (a group send is a real transmission per member over RF) and a progress line while it works. Groups also appear in the To field's autocomplete, expanded to their member list.
- **"Save this station" from a message.** The reader toolbar gets **📇+ Save Sender**, and the compose/reply To field gets a matching **📇+**, so a station can be filed while the message is still in front of you. Either opens the address book with the contact card over it, pre-filled with that callsign and (with QRZ connected) already looked up — so you can add a name, phone or group in the same breath. Both strip an `@route` / `@winlink.org` / hierarchical BBS suffix down to the bare callsign the address book is keyed on. A callsign already on file opens for editing rather than duplicating.
- **Subscribe / unsubscribe to bulletin TO fields.** Right-click (or long-press, on touch) a TO entry in the Bulletins tree, or any bulletin row in the message list, to unsubscribe from it — `WX`, `PKTNET`, and so on drop out of the tree's *ALL* roll-up, out of the message list, and out of the folder's unread badge. This is a local view filter only: nothing is killed or rejected on the BBS, unlike **Reject**, which asks the node itself to stop accepting the traffic. Unsubscribed TOs stay visible at the bottom of the tree, dimmed and marked ⊘, so they can be reviewed and resubscribed at any time; picking one explicitly still shows its mail. Persisted in `bpq_bull_unsub`.
- Long-press is now the touch equivalent of right-click for all three menus (iOS Safari never fires `contextmenu` on a long press). A press that moves is treated as a scroll and abandoned.

### Changed
- **The per-contact ✉ compose button in the list is much larger** (0.77rem → 1.7rem). It is the control reached for most often in that list and was previously a pinprick next to the delete ✕.
- **Import moved out of the address-book title bar** to sit beside **+ Add Contact** in the contact-list toolbar, where the other list actions are.
- **QRZ is no longer a section of the address book at all.** It was a full-width block at the top of the modal despite being one-time setup. The only entry point is now a small `🔍 look up on QRZ` text link in the contact detail pane: with credentials on file it just looks the callsign up, and without them it opens a compact "Connect QRZ.com" prompt naming the callsign and then *continues into the lookup* once you sign in — so connecting QRZ is something you do in passing the first time you need it, not a settings chore. The only always-visible trace is one dim `QRZ: <user>` / `QRZ not connected` link in the modal footer, which reopens the same prompt to change or disconnect the account.

### Fixed
- **Sending to a group popped one blocking OK dialog per recipient.** BPQ's `EMSave` reply page runs its own `alert()` (e.g. “N3HYM.#FRDK.MD.USA.NOAM added from WP”), and because that reply loads in a same-origin hidden iframe, every one of them blocked the whole window — a 23-recipient group meant 23 OK presses before the send finished. The send iframe is now sandboxed with `allow-forms allow-same-origin`: without `allow-scripts`/`allow-modals` the frame cannot open a dialog at all, while `allow-same-origin` keeps its body readable, so BPQ's message is recovered from the reply and folded into the single end-of-send toast (and logged in full to the console) rather than lost. One send now produces exactly one confirmation, whatever the recipient count.
- **A new contact card typed with a callsign that already existed silently overwrote that contact** instead of being refused. `saveContact()` decided "new versus edit" from the *typed* callsign rather than from how the form was opened, so a fresh card for an existing callsign fell through to the update path and wrote its blank fields over the saved record — losing a name, phone or notes with no warning and no undo. It now keys off how the card was opened, refuses a duplicate, and also refuses a rename onto a callsign that already exists (which would have merged two contacts into one).
- **The address book modal was pinned to 640px by an `#ab-modal` ID rule** in the `min-width: 769px` media query, left over from when it was a single narrow column. It outranked the new class-level width, so the three-pane layout was squeezed into a column about 68px wide.
- **Reply and Forward windows were titled "New Message".** `openCompose()` set the window title and then called `onComposeTypeChange()`, which unconditionally rewrites the title for the selected compose type — so the "Reply" / "Forward" title passed in was always immediately overwritten. The title is now applied after that call.
