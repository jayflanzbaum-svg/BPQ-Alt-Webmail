...
**Sync between your own PCs (optional).** Turn on **Sync to my other PCs** in the address-book footer and this PC keeps one hidden message to your own callsign holding its copy of your **contacts, groups, message templates, bulletin favourites/unsubscribes, signature and QTH**, and reads the ones written by your other machines — so a contact added or a topic starred on the shack PC turns up on the laptop with no export, import or file copying. Connection settings, passwords and display preferences (theme, font size, bold) are deliberately *not* synced: the first are per-machine, passwords must never sit in readable mail, and a phone and a desktop usually want different display settings. It never goes on the air: a message addressed to your own callsign at your home BBS stays on your node, and the carrier messages are hidden from every folder in this app (BPQ's own WebMail still shows them). Turn it on once per PC — and note each browser, and each address you open the app at, counts as a separate PC. Merging is per item, newest edit wins, and deletions stick rather than being resurrected by the other machine; the one case it cannot save you from is editing the *same* contact or template on two PCs before either has synced. Carrier size is capped at 60 KB, which is roughly 200 contacts with every field filled.
